...
const locale = require("./ui/app/localization/en");
...
createCopyTasks("images", {
  source: "./app/images/",
  destinations: commonPlatforms.map(platform => `./dist/${platform}/app/images`),
});
createCopyTasks("contractImages", {
  source: "./node_modules/eth-contract-metadata/images/",
  destinations: commonPlatforms.map(platform => `./dist/${platform}/app/images/contract`),
});
createCopyTasks("fonts", {
  source: "./app/fonts/",
  destinations: commonPlatforms.map(platform => `./dist/${platform}/app/fonts`),
});
...
gulp.task("build:typography", createScssBuildTask({
  src: "ui/app/css/typography.scss",
  dest: "ui/app/css/output",
  devMode: false,
}));

gulp.task("dev:typography", createScssBuildTask({
  src: "ui/app/css/typography.scss",
  dest: "ui/app/css/output",
  devMode: true,
  pattern: "ui/app/**/*.scss",
...
function zipTask (target) {
  return () => {
    return gulp.src(`dist/${target}/**`)
      .pipe(zip(`${locale.app.name}-${target}-${manifest.version}.zip`))
